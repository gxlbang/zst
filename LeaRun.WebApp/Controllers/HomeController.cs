...
namespace LeaRun.WebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult TreePage()
        {
            ...
            //取登录用户今天的订单信息
            var list = database.FindListBySql<Fx_Orders>("select Number,Stuts from Fx_Orders where UserId = '"
                + ManageProvider.Provider.Current().UserId + "' and CreateTime > '"
                +DateTime.Now.ToString("yyyy-MM-dd")+ " 00:00:00' and CreateTime < '" 
                +DateTime.Now.AddDays(1).ToString("yyyy-MM-dd")+ " 00:00:00'");
            ViewBag.TotalNum = list.Count;
            ViewBag.TotalNum1 = list.Where(o=>o.Stuts==1).ToList().Count;
            ViewBag.TotalNum2 = list.Where(o => o.Stuts == 2).ToList().Count;
            ViewBag.TotalNum3 = list.Where(o => o.Stuts == 3).ToList().Count;
            ViewBag.TotalNum4 = list.Where(o => o.Stuts == 4).ToList().Count;
            ViewBag.TotalNum5 = list.Where(o => o.Stuts == 5).ToList().Count;
            ViewBag.TotalNum6 = list.Where(o => o.Stuts == 6).ToList().Count;
            ViewBag.TotalNum7 = list.Where(o => o.Stuts == 7).ToList().Count;
            return View();
        }

        public JsonResult GetCurrMonthProSaleNum() {
            OrderCountBLL bll = new OrderCountBLL();
            var model = bll.GetOrderSaleInfo();
