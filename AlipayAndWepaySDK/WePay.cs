...
using AlipayAndWepaySDK.Interface;
...
namespace AlipayAndWepaySDK
{
    public class WePay : IWePay
    {
        ...
        /// <returns></returns>
        private static string BuildWatPay(string prepayid)
        {
            var dicParam = CreateWatPayParam(prepayid);
            string signString = CreateURLParamString(dicParam);
            string preString = signString + "&key=" + WepayConfig.WEPAY_WEB_APIPAYKEY;
        ...
        /// <param name="prepayId"></param>
        /// <returns></returns>
        private static SortedDictionary<string, string> CreateWatPayParam(string prepayId)
        {
            SortedDictionary<string, string> dic = new SortedDictionary<string, string>();
            dic.Add("appId", WepayConfig.WEPAY_WEB_APPID);//公众账号ID
            dic.Add("timeStamp", (Convert.ToInt32((DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, 0)).TotalSeconds)).ToString());//时间戳
            dic.Add("nonceStr", Guid.NewGuid().ToString().Replace("-", ""));//随机字符串
            dic.Add("package", "prepay_id="+ prepayId);//扩展字段
            dic.Add("signType", "MD5");//加密方式
            return dic;
        }
