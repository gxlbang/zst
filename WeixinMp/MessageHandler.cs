...
namespace Weixin.Mp.Sdk
{
    ...
    public class MessageHandler
    {
        ...
        /// <returns></returns>
        public static ReceiveMessageBase ConvertMsgToObject(string token, string msgBody = "")
        {
...
//            string msgBody = @"<xml><ToUserName><![CDATA[gh_3558vf9b2669]]></ToUserName>
//<FromUserName><![CDATA[otlhguCI1Znt1H2VdlHwTcze9AI4]]></FromUserName>
//<CreateTime>1396361902</CreateTime>
...
//</xml>";
            ...
            try
            {
                ...
                switch (strMsgType.Trim().ToLower())
                {
                    case "text":
                            ...
                            switch (eventNode.InnerText.Trim().ToLower())
                            {
                                case "subscribe":
                                    ...
                                    if (tmpNode != null)  // && (!string.IsNullOrWhiteSpace(tmpNode.InnerText)))  //&& tmpNode.InnerText.StartsWith("qrscene_")
                                    {
                                        ...
                                        scanSubEvt.CreateTime = msg.CreateTime;

                                        scanSubEvt.EventKey = rootElement.SelectSingleNode("EventKey") == null ? string.Empty : rootElement.SelectSingleNode("EventKey").InnerText;
                                        scanSubEvt.EventType = EventType.Subscribe;
                                        scanSubEvt.FromUserName = msg.FromUserName;
                                        scanSubEvt.MessageBody = msg.MessageBody;
                                        scanSubEvt.MsgType = msgType;
                                        scanSubEvt.ToUserName = msg.ToUserName;
                                        if (rootElement.SelectSingleNode("Ticket") != null)
                                        {
                                            scanSubEvt.Ticket = rootElement.SelectSingleNode("Ticket").InnerText;
                                        }
                                        else
                                        {
                                            scanSubEvt.Ticket = string.Empty;
                                        }
